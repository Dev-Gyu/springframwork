...
<tr>
	...
	<td><select name="jobId">
			...
			<c:if test="${emp.jobId == job.jobId}">
				<option value="${job.jobId}" selected>${job.jobId}: ${job.title}</option>
			</c:if>
			<c:if test="${emp.jobId != job.jobId}">
				<option value="${job.jobId}">${job.jobId}: ${job.title}</option>
			</c:if>
			</c:forEach>
	</select></td>
</tr>
...
<tr>
	...
	<td><select name="departmentId">
		...
		<c:if test="${emp.departmentId == department.departmentId}">
			<option value="${department.departmentId}" selected>${department.departmentId}: ${department.departmentName}</option>
		</c:if>
		<c:if test="${emp.departmentId != department.departmentId}">
